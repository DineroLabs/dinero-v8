...
struct WalletUTXO { 
    ...
    UTXO() : vout(0), value(0), spendable(false), locked(false) {}
    UTXO(const std::string& tx, uint32_t v, int64_t val, bool spend, bool lock)
...
struct IWallet {
    ...
    // UTXO management
    virtual std::vector<UTXO> listUnspent(int min_conf = 1, int max_conf = 999999) const = 0;
    ...
    virtual FundResult createFundedPsbt(const std::vector<UTXO>& inputs,
