...
public:
    using MessageHandler = std::function<void(std::shared_ptr<Peer>, MessageType, const std::vector<uint8_t>&)>;
    using DisconnectHandler = std::function<void(std::shared_ptr<Peer>, DisconnectReason)>;
    
    // Constructor for outbound connections
    Peer(boost::asio::io_context& io_context, const std::string& address, uint16_t port);
    
    // Constructor for inbound connections
    Peer(boost::asio::tcp::socket socket);
