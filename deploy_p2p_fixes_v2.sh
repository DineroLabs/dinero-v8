...
deploy_to_server() {
    ...
    # Rebuild only the RPC handler library and daemon binary
    echo "  2️⃣  Rebuilding daemon (ignoring test failures)..."
    ssh -i "$SSH_KEY" root@$SERVER "
        cd ~/DineroCoin/build
        # Build the RPC handler library and daemon binary
        make dinero_rpc_handlers dinerod -j\$(nproc) 2>&1 | tail -30 || true
    "
    ...
    ssh -i "$SSH_KEY" root@$SERVER "cd ~/DineroCoin/build && nohup bin/dinerod -daemon -addnode=$OTHER_SERVER:20999 > /tmp/dinerod_debug.log 2>&1 &"
...
# Deploy to Server 1
deploy_to_server "$SERVER1" "Server 1" "$SERVER2"

# Deploy to Server 2
deploy_to_server "$SERVER2" "Server 2" "$SERVER1"
