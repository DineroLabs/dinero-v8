...
# GUI Integration Test (Fixed)
# Tests the complete integrated system with real blockchain data

set -e

echo "🧪 GUI Integration Testing Suite (Fixed)"
echo "========================================="
echo ""
...
test_daemon_startup() {
    ...
    cd "$DINERO_ROOT"
    timeout $TIMEOUT ./build/dinerod $network_flag --datadir="$TEST_DATADIR" --daemon &
    DAEMON_PID=$!
    ...
    if kill -0 $DAEMON_PID 2>/dev/null; then
        ...
        # Determine correct port for network
        local port=20999  # mainnet default
        if [ "$network" = "regtest" ]; then
            port=20999  # regtest uses same port in unified mode
        elif [ "$network" = "testnet" ]; then
            port=20999  # testnet uses same port in unified mode
        fi
        
        # Test RPC connection with a longer wait
        sleep 5
        if timeout 15 curl -s "http://localhost:$port/api/v1/getblockchaininfo" >/dev/null; then
            log_success "RPC endpoint responding for $network on port $port"
        else
            log_warning "RPC endpoint not responding for $network on port $port"
        fi
...
test_database_integrity() {
    ...
    # Determine correct subdirectory based on network
    local subdir="mainnet"  # default
    if [ "$network" = "regtest" ]; then
        subdir="regtest"
    elif [ "$network" = "testnet" ]; then
        subdir="testnet"
    ...
    local db_path="$TEST_DATADIR/$subdir/blockchain.db"
    ...
    if [ -f "$db_path" ]; then
        ...
        # Try to create database by running daemon briefly
        log_info "Attempting to create database by running daemon..."
        ...
        cd "$DINERO_ROOT"
        timeout 15 ./build/dinerod $network_flag --datadir="$TEST_DATADIR" --daemon &
        DAEMON_PID=$!
        sleep 10
        kill $DAEMON_PID 2>/dev/null || true
        wait $DAEMON_PID 2>/dev/null || true
        ...
        if [ -f "$db_path" ]; then
            log_success "Database created successfully for $network"
        else
            log_warning "Database still not found after daemon run"
        fi
    fi
}
...
test_rpc_endpoints() {
    local network=$1
    local port=20999  # unified port mode
...
test_gui_startup() {
    ...
    # Build GUI first if not exists
    if [ ! -f "./build/dinero-desktop" ]; then
        log_info "GUI binary not found, attempting to build..."
        cd build
        if make dinero-desktop >/dev/null 2>&1; then
            log_success "GUI binary built successfully"
        else
...
test_network_switching() {
    ...
    # Test that different networks use different data directories
    for network in "${NETWORKS[@]}"; do
        # Determine correct flag and expected directory
        local network_flag=""
        local expected_subdir="mainnet"
        
        if [ "$network" = "regtest" ]; then
            network_flag="--regtest"
            expected_subdir="regtest"
        elif [ "$network" = "testnet" ]; then
            network_flag="--testnet"
            expected_subdir="testnet"
        fi
        
        local expected_dir="$TEST_DATADIR/$expected_subdir"
        ...
        if kill -0 $DAEMON_PID 2>/dev/null; then
            ...
            if [ -d "$expected_dir" ]; then
                log_success "Network-specific directory created for $network: $expected_subdir"
            else
                log_warning "Network-specific directory not found for $network: $expected_dir"
            fi
...
# Main test execution
main() {
    log_info "Starting GUI Integration Testing Suite (Fixed)..."
    echo ""
    ...
    # Test 5: Database Integrity (after daemon startup to ensure DBs exist)
    for network in "${NETWORKS[@]}"; do
