...
ENVELOPE_VERSION = 2
OUTGOING_PLAINTEXT_VERSION = 1
OUTGOING_PLAINTEXT_BYTES = 97
OUTGOING_CIPHERTEXT_BYTES = OUTGOING_PLAINTEXT_BYTES + 16
V2_ENVELOPE_BYTES = 1 + LEGACY_ENCRYPTED_NOTE_BYTES + OUTGOING_CIPHERTEXT_BYTES
...
def auth_note_commitment(d: bytes, pk_d_spend: bytes, value: int, rcm: bytes) -> bytes:
    if len(d) != 11 or len(pk_d_spend) != 32 or len(rcm) != 32:
        raise ValueError("invalid authenticated-note component size")
    d_packed = d.ljust(32, b"\0")
    address_key = base.poseidon(d_packed, pk_d_spend)
...
def outgoing_plaintext(pk_d_enc: bytes, pk_d_spend: bytes, esk: bytes) -> bytes:
    if len(pk_d_enc) != 32 or len(pk_d_spend) != 32 or len(esk) != 32:
        raise ValueError("invalid outgoing plaintext component size")
    return bytes([OUTGOING_PLAINTEXT_VERSION]) + pk_d_enc + pk_d_spend + esk
...
def encrypt_outgoing(ovk: bytes, public: PublicOutput, recipient_note: bytes,
    ...
    if len(envelope) != V2_ENVELOPE_BYTES:
        raise AssertionError("v2 envelope length drift")
    return envelope, {
...
def recover(ovk: bytes, public: PublicOutput, encoded: bytes, *, height: int,
    ...
    if len(encoded) != V2_ENVELOPE_BYTES:
    ...
    expected_commitment = auth_note_commitment(d, pk_d_spend, value, rcm)
    ...
    return RecoveryResult(Verdict.OK, {
        ...
        "recipient_address_payload": d + pk_d_enc + pk_d_spend,
...
def build_vectors() -> dict[str, Any]:
    ...
    commitment = auth_note_commitment(d, pk_d_spend, value, rcm)
    ...
    plaintext = outgoing_plaintext(pk_d_enc, pk_d_spend, esk)
    ...
    zero_esk_plaintext = plaintext[:65] + b"\0" * 32
    order_esk_plaintext = plaintext[:65] + base.b32(base.N)
    mismatched_esk = (int.from_bytes(esk, "big") + 1) % base.N
    if mismatched_esk == 0:
        mismatched_esk = 1
    mismatched_esk_plaintext = plaintext[:65] + base.b32(mismatched_esk)
    ...
    noncanonical_esk_plaintext = plaintext[:65] + base.b32(
    ...
    failures = {
        ...
        "truncated_v2": Verdict.INVALID_LENGTH,
        "trailing_v2": Verdict.INVALID_LENGTH,
    ...
    return {
        "format": "dinero-shielded-outgoing-view-v2-independent-vectors",
        "version": 1,
        ...
        "sizes": {
            ...
            "v2_envelope": V2_ENVELOPE_BYTES,
            "v2_envelope_compact_size": base.compact_size(V2_ENVELOPE_BYTES).hex(),
        },
        "domains": {
        ...
        "source": {
            ...
            "pk_d_spend": h(pk_d_spend), "esk_normalized": h(esk),
        ...
        "compatibility": {
            ...
            "v2_before_spend_authority": "must not be emitted and is rejected by upgraded wallets",
            "v2_after_both_activations": (
                "incoming viewing and outgoing recovery are available to upgraded wallets"),
            "old_wallet_receiving_v2": (
        ...
        "mutation_fixtures": {
            "truncated_v2": h(envelope[:-1]),
            "trailing_v2": h(envelope + b"\0"),
            "unknown_envelope_version": h(bytes([3]) + envelope[1:]),
            "tampered_outgoing_tag": h(envelope[:-1] + bytes([envelope[-1] ^ 1])),
            "unknown_plaintext_version": h(reencrypt_with_plaintext(
                ovk, public, envelope, b"\x02" + plaintext[1:])),
...
def exercise_failure_classes(vectors: dict[str, Any]) -> dict[str, str]:
    ...
    envelope = bytes.fromhex(vectors["outgoing"]["envelope_v2"])
    ...
    return {
        ...
        "truncated_v2": run(bytes.fromhex(mutations["truncated_v2"])),
        "trailing_v2": run(bytes.fromhex(mutations["trailing_v2"])),
...
def self_test(vectors: dict[str, Any]) -> None:
    ...
    envelope = vectors["outgoing"]["envelope_v2"]
    mutations = vectors["mutation_fixtures"]
    byte_mutants = [value for key, value in mutations.items()
                    if key.endswith("v2") or key.endswith("version") or
    ...
    if len(raw_envelope) != V2_ENVELOPE_BYTES or raw_envelope[0] != ENVELOPE_VERSION:
        raise AssertionError("v2 envelope version or length drift")
    if raw_envelope[1:1 + LEGACY_ENCRYPTED_NOTE_BYTES] != recipient:
        raise AssertionError("v2 did not embed the legacy recipient ciphertext verbatim")
    if vectors["sizes"]["v2_envelope_compact_size"] != "fdd502":
        raise AssertionError("725-byte CompactSize encoding drift")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=pathlib.Path("tests/vectors/shielded_outgoing_view_v2_external.json"))
