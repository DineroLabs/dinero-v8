...
ctest --test-dir "${build_dir}" --no-tests=error --output-on-failure -j 1 \
    -R '^(P2PHeaderParserAlignment|DaemonServiceRelease|PackedHeaderAlignment|SerializationEmptyBuffers|CompactRegtestFixedVectors|CompactRegtestVectorOracle|ShieldedResourceLimits|ShieldedReindexEquivalence|ShieldedAuthRelayLifecycle|CompactRegtestLifecycle|CsnManualInvalidation|CSNShieldedReorgInvertibility)$' \
