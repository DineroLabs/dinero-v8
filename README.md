...
<p align="center">
  <a href="https://github.com/DineroLabs/dinero-v8/releases/latest"><img src="https://img.shields.io/badge/release-v8.1.12-blue" alt="Release"></a>
...
| **AssumeUTXO fast sync** | Content-hash-pinned snapshot anchors bring a new node to the tip quickly, then validate history forward in the background. |
...
docker run -d --name dinero --stop-timeout 60 \
  ...
  -v dinero-data:/data -p 20999:20999 ghcr.io/dinerolabs/dinero-v8:8.1.12
```
...
```bash
docker build -t ghcr.io/dinerolabs/dinero-v8:8.1.12 .
```

That builds the v8.1.12 image the workflow publishes today, under the same name the
command above uses; pass `--build-arg DINERO_VERSION=<version>` for any other release.

## Current Downloads

Use the current stable `v8.1.12` release from:
