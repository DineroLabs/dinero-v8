...
DATA_DIR="/tmp/dinero_tip_persist_restart_equiv_$$"
...
assert_sync_state() {
    ...
    jq -e \
        ...
        .forest_tip_marker_hash == .active_best_hash and
        (.forest_tip_marker_root | type) == "string" and
        (.forest_tip_marker_root | length) > 0
...
info "Building a checkpointed chain before the tip-persist crash"
mine_blocks 24 "${MINER_ADDR}"
wait_for_sync_state 24 "pre-crash steady state"
pass "Pre-crash chain reached height 24 with checkpoint + marker aligned"

stop_node

start_node "${LOG_CRASH}" DINERO_CRASH_AT=after_tip_before_checkpoint
wait_rpc || fail "crash-test daemon did not reach RPC readiness"
assert_sync_state 24 "pre-trigger state on crash daemon"

info "Triggering crash after tip persistence but before checkpoint catch-up"
set +e
...
wait_dead "${PID}" || fail "daemon did not crash at after_tip_before_checkpoint"
PID=""

grep -q "DINERO_CRASH" "${LOG_CRASH}" || fail "crash log did not show named crash hook"
pass "Crash hook triggered after canonical tip persistence"

start_node "${LOG_RESTART}"
wait_rpc || fail "restarted daemon did not reach RPC readiness"
wait_for_sync_state 25 "post-restart replay to persisted tip"
pass "Restart replayed forward to the persisted tip and restored checkpoint state"

HEIGHT_RESULT="$(rpc_call "${DATA_DIR}" "getblockcount" '[]')"
rpc_has_error "${HEIGHT_RESULT}" && fail "getblockcount failed after restart: ${HEIGHT_RESULT}"
[[ "$(jq -r '.result' <<<"${HEIGHT_RESULT}")" == "25" ]] || fail "unexpected height after restart: ${HEIGHT_RESULT}"
...
[[ -n "$(jq -r '.result // empty' <<<"${HASH_25_RESULT}")" ]] || fail "height 25 hash missing after restart"

mine_blocks 1 "${MINER_ADDR}"
wait_for_sync_state 26 "post-restart mining"
pass "Post-restart mining preserved tip/checkpoint/marker equivalence"
