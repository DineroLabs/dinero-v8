...
info "Building a checkpointed chain before marker-clear crash"
...
start_node "${LOG_CRASH}" DINERO_CRASH_AT=after_utreexo_checkpoint_before_marker_clear
wait_rpc || fail "crash-test daemon did not reach RPC readiness"
wait_for_sync_state 24 "pre-trigger state on crash daemon"

info "Triggering crash after checkpoint persistence but before advisory marker clear"
set +e
...
wait_dead "${PID}" || fail "daemon did not crash at after_utreexo_checkpoint_before_marker_clear"
PID=""

grep -q "DINERO_CRASH" "${LOG_CRASH}" || fail "crash log did not show named crash hook"
[[ -f "${DATA_DIR}/chainstate_recovery.marker" ]] || fail "durable recovery marker missing after crash"
pass "Crash left the durable recovery marker behind as expected"

start_node "${LOG_RESTART}"
wait_rpc || fail "restarted daemon did not reach RPC readiness"
wait_for_sync_state 25 "post-restart state"
[[ ! -f "${DATA_DIR}/chainstate_recovery.marker" ]] || fail "stale recovery marker still present after healthy restart"
pass "Healthy restart cleared the stale recovery marker"
