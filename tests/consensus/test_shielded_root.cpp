...
TEST(ShieldedRoot, PreimageStartsWithTagAndVersion) {
    const auto pre = BuildShieldedRootPreimage(Root32(0xAA), 7, Acc({1, 2}), {3, 4, 5});
...
TEST(ShieldedRoot, NonStandardRootLengthIsZeroFilledNotSkipped) {
    const auto short_root = BuildShieldedRootPreimage({1, 2, 3}, 0, Acc({}), {});
    const auto zero_root = BuildShieldedRootPreimage(std::vector<uint8_t>(32, 0), 0, Acc({}), {});
    EXPECT_EQ(short_root, zero_root);
    EXPECT_EQ(short_root.size(), 4u + 1u + 32u + 8u + 32u + 8u);
}

// ── every field is actually covered ───────────────────────────────────────

TEST(ShieldedRoot, EveryFieldChangesTheDigest) {
    const auto base = ComputeShieldedRootFromParts(Root32(0x11), 5, Acc({9}), {8});
    EXPECT_NE(base, ComputeShieldedRootFromParts(Root32(0x12), 5, Acc({9}), {8})) << "tree root";
    EXPECT_NE(base, ComputeShieldedRootFromParts(Root32(0x11), 6, Acc({9}), {8})) << "tree size";
    EXPECT_NE(base, ComputeShieldedRootFromParts(Root32(0x11), 5, Acc({7}), {8})) << "nullifiers";
    EXPECT_NE(base, ComputeShieldedRootFromParts(Root32(0x11), 5, Acc({9}), {6})) << "anchors";
}

TEST(ShieldedRoot, IsDeterministic) {
    EXPECT_EQ(ComputeShieldedRootFromParts(Root32(0x11), 5, Acc({9}), {8}),
              ComputeShieldedRootFromParts(Root32(0x11), 5, Acc({9}), {8}));
}
...
TEST(ShieldedRoot, SectionBoundaryIsUnambiguous) {
    ...
    const auto a = ComputeShieldedRootFromParts(Root32(0), 0, Acc({1, 2}), {3});
    const auto b = ComputeShieldedRootFromParts(Root32(0), 0, Acc({1}), {2, 3});
...
TEST(ShieldedRoot, EmptySectionsAreDistinctFromAbsentOnes) {
    const auto empty_nf = ComputeShieldedRootFromParts(Root32(0), 0, Acc({}), {1});
    const auto empty_ah = ComputeShieldedRootFromParts(Root32(0), 0, Acc({1}), {});
...
TEST(ShieldedRoot, DigestDependsOnlyOnTheFourShieldedInputs) {
    ...
    const auto x = ComputeShieldedRootFromParts(Root32(0x33), 42, Acc({1, 2, 3}), {4, 5});
    const auto y = ComputeShieldedRootFromParts(Root32(0x33), 42, Acc({1, 2, 3}), {4, 5});
    EXPECT_EQ(x, y);
}
...
TEST(ShieldedRoot, OmittingAnyNullifierChangesTheRoot) {
    ...
    const auto honest = ComputeShieldedRootFromParts(Root32(1), 4, Acc(full), {9, 9});
    for (size_t drop = 0; drop < full.size(); ++drop) {
        std::vector<uint8_t> tampered = full;
        tampered.erase(tampered.begin() + static_cast<long>(drop));
        EXPECT_NE(honest, ComputeShieldedRootFromParts(Root32(1), 4, Acc(tampered), {9, 9}))
...
TEST(ShieldedRoot, TruncatedNullifierSetIsDetected) {
    ...
    const auto honest = ComputeShieldedRootFromParts(Root32(1), 4, Acc(full), {7});
    for (size_t keep = 0; keep < full.size(); ++keep) {
        const std::vector<uint8_t> shorter(full.begin(), full.begin() + static_cast<long>(keep));
        EXPECT_NE(honest, ComputeShieldedRootFromParts(Root32(1), 4, Acc(shorter), {7}))
...
TEST(ShieldedRoot, InjectedAnchorChangesTheRoot) {
    ...
    const auto honest = ComputeShieldedRootFromParts(Root32(1), 4, Acc({5}), anchors);
    std::vector<uint8_t> injected = anchors;
    injected.push_back(0xFF);  // one extra anchor entry
    EXPECT_NE(honest, ComputeShieldedRootFromParts(Root32(1), 4, Acc({5}), injected));
}
...
TEST(ShieldedRoot, ReorderedNullifierContentChangesTheRoot) {
    const auto ordered = ComputeShieldedRootFromParts(Root32(1), 4, Acc({1, 2, 3, 4}), {9});
    const auto swapped = ComputeShieldedRootFromParts(Root32(1), 4, Acc({3, 4, 1, 2}), {9});
...
TEST(ShieldedRoot, ContentCannotBeShuffledBetweenSections) {
    const auto a = ComputeShieldedRootFromParts(Root32(1), 4, Acc({1, 2, 3, 4}), {5, 6});
    const auto b = ComputeShieldedRootFromParts(Root32(1), 4, Acc({1, 2, 3}), {4, 5, 6});
    const auto c = ComputeShieldedRootFromParts(Root32(1), 4, Acc({1, 2, 3, 4, 5}), {6});
    EXPECT_NE(a, b);
    EXPECT_NE(a, c);
    EXPECT_NE(b, c);
}
...
TEST(ShieldedRoot, EmptyStateDigestIsPinned) {
    const auto d = ComputeShieldedRootFromParts(std::vector<uint8_t>(32, 0), 0, Acc({}), {});
