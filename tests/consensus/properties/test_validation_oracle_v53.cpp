...
#include "validation_oracle_v53.h"
#include <random>

using namespace dinero;
using namespace dinero::consensus::test;

class ValidationOracleV53Test : public ::testing::Test {
protected:
...
TEST_F(ValidationOracleV53Test, EmptyTrace_NoViolations) {
    ValidationOracleV53 oracle;
    ValidationTrace trace(42, "empty");
    auto violations = oracle.check(trace);
    EXPECT_EQ(violations.size(), 0);
}

TEST_F(ValidationOracleV53Test, ValidPath_NoViolations) {
    ValidationOracleV53 oracle;
    ...
    auto violations = oracle.check(trace);
    EXPECT_EQ(violations.size(), 0) << "Valid block should have no violations";
}

TEST_F(ValidationOracleV53Test, InvalidInput_TriggersViolation) {
    ValidationOracleV53 oracle;
    ...
    validation_fail.error_message = "Invalid proof of work";
    trace.events.push_back(validation_fail);

    // Event: But then it's somehow connected (violation!)
    ...
    auto violations = oracle.check(trace);
    EXPECT_GT(violations.size(), 0) << "Failed block with side effects should be detected";
    if (!violations.empty()) {
        EXPECT_EQ(violations[0].property, "V5.3");
    }
}

TEST_F(ValidationOracleV53Test, MixedValidInvalid_DetectsOnlyInvalid) {
    ValidationOracleV53 oracle;
    ...
    invalid_fail.error_message = "Bad timestamp";
    trace.events.push_back(invalid_fail);

    // Invalid block somehow connected (violation!)
