#!/bin/bash

# 🚀 Dinero Cryptocurrency - Linux Build Script
# This script builds Dinero on Linux (Ubuntu/Debian/CentOS/RHEL)

set -e  # Exit on any error

echo "🚀 Building Dinero Cryptocurrency on Linux..."
echo "=============================================="

# Detect package manager
if command -v apt-get &> /dev/null; then
    PKG_MANAGER="apt"
    echo "📦 Detected package manager: apt (Ubuntu/Debian)"
elif command -v yum &> /dev/null; then
    PKG_MANAGER="yum"
    echo "📦 Detected package manager: yum (CentOS/RHEL)"
elif command -v dnf &> /dev/null; then
    PKG_MANAGER="dnf"
    echo "📦 Detected package manager: dnf (Fedora/RHEL)"
else
    echo "❌ Unsupported package manager. Please install dependencies manually."
    exit 1
fi

# Install dependencies
echo "🔍 Installing dependencies..."
if [ "$PKG_MANAGER" = "apt" ]; then
    sudo apt-get update
    sudo apt-get install -y build-essential cmake git libssl-dev libboost-all-dev qt6-base-dev qt6-declarative-dev
elif [ "$PKG_MANAGER" = "yum" ]; then
    sudo yum groupinstall -y "Development Tools"
    sudo yum install -y cmake git openssl-devel boost-devel qt6-qtbase-devel qt6-qtdeclarative-devel
elif [ "$PKG_MANAGER" = "dnf" ]; then
    sudo dnf groupinstall -y "Development Tools"
    sudo dnf install -y cmake git openssl-devel boost-devel qt6-qtbase-devel qt6-qtdeclarative-devel
fi

# Check prerequisites
echo "🔍 Checking prerequisites..."

# Check if g++ is installed
if ! command -v g++ &> /dev/null; then
    echo "❌ C++ compiler not found!"
    exit 1
fi

# Check if CMake is installed
if ! command -v cmake &> /dev/null; then
    echo "❌ CMake not found!"
    exit 1
fi

# Check if Qt6 is available (optional)
QT_PATH=""
if [ -d "/usr/lib/x86_64-linux-gnu/qt6" ]; then
    QT_PATH="/usr/lib/x86_64-linux-gnu/qt6"
    echo "✅ Found Qt6 at: $QT_PATH"
elif [ -d "/usr/lib64/qt6" ]; then
    QT_PATH="/usr/lib64/qt6"
    echo "✅ Found Qt6 at: $QT_PATH"
else
    echo "⚠️  Qt6 not found - GUI components will not be built"
    echo "   Install Qt6 for GUI support"
fi

echo "✅ Prerequisites check passed!"

# Create build directory
echo "📁 Creating build directory..."
rm -rf build
mkdir -p build
cd build

# Configure build
echo "⚙️  Configuring build..."
if [ -n "$QT_PATH" ]; then
    cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH="$QT_PATH" -DENABLE_SANITIZERS=OFF
else
    cmake .. -DCMAKE_BUILD_TYPE=Release -DENABLE_SANITIZERS=OFF -DBUILD_GUI=OFF
fi

# Build
echo "🔨 Building Dinero..."
cmake --build . -j$(nproc)

echo "✅ Build completed successfully!"
echo ""
echo "🚀 Your Dinero binaries are ready in: ./build/bin/"
echo ""
echo "📋 Next steps:"
echo "1. Create data directory: mkdir -p data"
echo "2. Start daemon: ./build/bin/dinerod -datadir=./data -rpcport=20998 -port=20999 -daemon=0 -server=1"
echo "3. Create wallet and start mining!"
echo ""
echo "📚 See README.md for complete usage instructions"
echo "⚡ See QUICK_START.md for essential commands"
