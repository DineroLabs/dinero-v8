...
PrivateCovenantWidget::PrivateCovenantWidget(RpcClient* rpc,QWidget* parent):QWidget(parent),rpc_(rpc) {
    ...
    auto* intro=new QLabel(tr("Private covenants pay one or two fixed shielded recipients. Amounts and recipients are hidden; the earliest spend height and fee are public. Your wallet recovers the encrypted contract from its funding note. A public funding source reveals the amount entering the shielded pool."));
    intro->setWordWrap(true); layout->addWidget(intro);
    status_=new QLabel(tr("Waiting for network capability")); status_->setWordWrap(true); layout->addWidget(status_);
    ...
    owner_->setPlaceholderText(tr("Your shielded address; its wallet will control the contract"));
    form->addRow("Contract owner:",owner_);
    auto* ownAddress=new QPushButton(tr("Use my shielded address"));
    ...
    connect(ownAddress,&QPushButton::clicked,this,[this]{
        if(scope_.isEmpty()) { ownerStatus_->setText(tr("Select or load a wallet first.")); return; }
        ownerStatus_->setText(tr("Getting your shielded address…"));
    ...
    height_->setToolTip(tr("Absolute block height, not a duration. Zero permits spending after confirmation."));
    form->addRow("Earliest spend height:",height_);
    fee_=new QLineEdit("0.01000000"); fee_->setObjectName("privateCovenantFee");
    fee_->setToolTip(tr("Reserved in the contract. The exact payment outputs cannot be reduced later to raise this fee. Funding fee is additional and estimated by the daemon."));
    ...
    fundingFee_->setToolTip(tr("Exact fee paid to fund this contract. If below relay policy, the daemon rejects the transaction without raising the fee."));
    ...
    auto* refreshButton=new QPushButton(tr("Refresh private contracts"));
    ...
    connect(resolve_,&QPushButton::clicked,this,[this]{
        ...
        if(QMessageBox::warning(this,tr("Resolve uncertain submission"),
            tr("First check transaction history and refresh recovered contracts. The previous transaction may already have reached the network.\n\n"
        ...
        if(settings.status()!=QSettings::NoError) { status_->setText(tr("Cannot save resolution; the hold remains.")); return; }
        uncertain_=false;
        for(int row=0;row<2;++row) for(int column=0;column<2;++column) outputs_->item(row,column)->setText({});
        status_->setText(tr("Previous outcome reviewed. Compose a new operation if needed; nothing was resubmitted.")); updateEnabled();
    });
    ...
    connect(rpc_,&RpcClient::rpcResult,this,[this](const QString& method,const QJsonValue& result){
        ...
        if(method=="wallet.getshieldedaddress" && !ownerRequestScope_.isEmpty() && ownerRequestScope_==scope_) {
            ...
            if(!address.isEmpty() && (!object.contains("error") || object.value("error").isNull())) { owner_->setText(address); ownerStatus_->setText(tr("Your shielded address is ready.")); }
            ...
            if(!error.isEmpty()) {
                ...
                uncertain_=true; status_->setText(tr("No transaction ID returned. Outcome uncertain; check recovered contracts before taking further action."));
            }
...
void PrivateCovenantWidget::render() {
    ...
    for(const auto& value:notes_) {
        ...
        auto* button=new QPushButton(tr("Review payment"));
...
void PrivateCovenantWidget::fund() {
    ...
    if(!ShieldedTransferPolicy::parseDinToUna(fee_->text(),&fee) || fee<=0 || !ShieldedTransferPolicy::parseDinToUna(fundingFee_->text(),&fundingFee) || owner_->text().trimmed().isEmpty()) {
        status_->setText(tr("Enter a contract owner and positive funding and reserved spend fees.")); return;
    }
    ...
    for(int r=0;r<2;++r) {
        ...
        qint64 value=0;
        if(address.isEmpty() || !CovenantFormPolicy::appendAmount(amount,total,value)) { status_->setText(tr("Complete each recipient row with a positive amount of at most 8 decimals.")); return; }
        outputs.append(QJsonObject{{"address",address},{"value_una",value}});
        review.append(address+"  "+CovenantFormPolicy::formatUna(value)+" DIN");
    }
    if(outputs.isEmpty() || total>std::numeric_limits<qint64>::max()-fee || fundingFee>std::numeric_limits<qint64>::max()-fee-total) { status_->setText(tr("Enter one or two valid payments within the amount range.")); return; }
    ...
    if(QMessageBox::question(this,tr("Review private covenant"),text,QMessageBox::Ok|QMessageBox::Cancel,QMessageBox::Cancel)!=QMessageBox::Ok) return;
...
void PrivateCovenantWidget::spend(const QJsonObject& note) {
    ...
    if(QMessageBox::question(this,tr("Review private covenant payment"),text,QMessageBox::Ok|QMessageBox::Cancel,QMessageBox::Cancel)!=QMessageBox::Ok) return;
    if(reviewedScope!=journalKey()) return;
...
void PrivateCovenantWidget::submit(const QString& method,const QJsonObject& params) {
    ...
    if(settings.status()!=QSettings::NoError) { status_->setText(tr("Cannot save the operation journal; nothing submitted.")); return; }
    pendingMethod_=method; status_->setText(tr("Building and submitting proof. Please wait…")); updateEnabled(); rpc_->callNamed(method,params);
}
