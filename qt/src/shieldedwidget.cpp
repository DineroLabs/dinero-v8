...
void ShieldedWidget::setupUI() {
    ...
    auto* transferBox = new QGroupBox("Send shielded (to dins/tdins/rdins address)");
    {
        ...
        transferFeeEdit_ = new QLineEdit(QString::number(kDefaultFeeUna));
...
void ShieldedWidget::setActiveBanner(bool active, const QString& reason) {
    ...
    if (transferBtn_)  transferBtn_->setEnabled(active);
...
void ShieldedWidget::onTransferClicked() {
    ...
    qint64 fee = transferFeeEdit_->text().toLongLong(&ok);
    if (!ok || fee <= 0) fee = kDefaultFeeUna;
    QJsonObject p{
        {"fee_una",    fee},
    ...
    transferResultLabel_->setText("submitting…");
}
