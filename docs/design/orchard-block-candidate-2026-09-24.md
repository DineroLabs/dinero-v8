...
The staged coin/state entry points require the existing 100,000-byte transaction, 400,000 transaction-weight, 1,000,000 block-base and 4,000,000 block-weight bounds before coin lookup or proof authorization. Tests cover aggregates crossing the base and weight limits separately, full suffix accounting and rejection before an intentionally failing coin lookup. The mixed coin connector also requires a single historical coinbase with a 2..100-byte script and the minimal positive height prefix produced by the shared miner Script encoder. This runs before coin resolution/proof work. It applies only to the staged post-activation profile; historical replay rules are unchanged.

An Orchard proof-work budget, complete sigop accounting and contextual header/PoW checks remain integration obligations.
