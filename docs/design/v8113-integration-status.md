...
| Wallet keys, addresses, proving, shield/send/unshield | Staged ZIP32 keys/receivers, note reception, incremental witnesses and fresh shield/send/unshield proofs; canonical witness persistence and encrypted snapshot storage staged; typed scan/checkpoint restore implemented as a component; origin callbacks, address counters, operation job/archival callbacks and live wallet integration not implemented; Reserved/Ready persistence implemented as a component |
