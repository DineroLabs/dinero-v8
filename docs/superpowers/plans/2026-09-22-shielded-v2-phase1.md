...
|---|---|---|---|---|
| single verify | one 2-in-2-out v7 tx, fresh proof | **hardware contract to be fixed by the owner**: spec §2 names "fleet 8-core x86", spec §6 names the CI runner; M4 Max and the 2-core GitHub VM are both recorded, neither is the contract | cold (proof never seen; cache bypassed) | ≤ 20 ms |
| block verify | 50 × 2-in-2-out v7 txs, fresh proofs, one bounded worker budget (1/2/4/8 measured) | same contract | cold | ≤ 400 ms |
| prove | one 2-in-2-out v7 tx | same contract | n/a | ≤ 600 ms |
...
**Status 2026-09-22, third pass (`docs/benchmarks/shielded-v2-task7-csr-oldvsnew-20260922.md`):** per-shape CSR matrix walk lands 17% serial / 5% 8-thread / 15% batch on M4 (28.4 ms serial, 18.1 ms 8-thread, 223 ms per 50 on 8 workers; 1/2/4/8 workers = 1,493/750/400/223 ms); old design on the same host ≈ 3.4 s verify per 2-in-2-out tx (lower bound, cv-bound profile) against 28 ms. Large-fixture (20k constraints) differential regression covers the parallel branch on valid and invalid proofs. Hardware contract still the owner's to fix. **Status 2026-09-22, second pass after the owner's review of 6990a3168 (`docs/benchmarks/shielded-v2-task7-entrypoint-20260922.json`):** legacy decoder behaviour restored (trailing bytes accepted for the legacy profile, rejected only for the E-less profile; six differential cases in `tests/zk/test_spartan_profile_compat.cpp`); the matrix worker count is an explicit verifier parameter defaulting to serial; the batch uses one budget of 8 workers with a serial inner path. Complete warm entry point for 2-in-2-out (deserialize + trusted structure copy with inputs + verify): **18.8 ms with 8 matrix threads, 33.8 ms serial**; cold init per shape 50.9 ms; 50 proofs on 8 workers 263 ms. The single-verify gate therefore holds only when the verifier is given 8 cores; the block gate holds with a serial inner path. Linux x86 numbers come from the `shielded-v2-spike-bench` workflow artifact. Earlier status (superseded):  the tuning below is implemented on `claude/shielded-v2` in `src/zk/zkvm/r1cs_spartan.{h,cpp}` (additive `omit_error_term` profile, parallel row-wise M̃ evaluation, structure hash treated as a per-shape constant) and measured on the exact §10.2 statement: 2-in-2-out **17.4 ms** verify (gate 20), **317.7 ms** per block of 50 on 8 threads (gate 400), 11,771 B (gate 32,768), 485.7 ms prove (gate 600). Before tuning: 70.2 / 643.5 / 19,749 / 523.6. Seven profile-soundness checks and the legacy `test_spartan_soundness` (18/18) pass. The margin on the single-verify gate is 13% and depends on 8 idle cores; the remaining task work is wiring the cached verifier structure (copy + `set_value` 2.8 ms vs 13.3 ms rebuild) into `VerifyBundleV2`, the `SpartanProfile::BundleV2` naming, the soundness gtest, and the gate ctest. Nothing here changes the security scope (§10.7).
