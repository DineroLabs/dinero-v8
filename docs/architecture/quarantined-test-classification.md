...
1. Repair the four individually quarantined `WalletMainnetReadiness_*` hardening
   subcases one at a time.
2. Repair `ConsensusFormalVerification` as a consensus-test-vector/formula PR.
3. Keep `UtreexoE2ERelay` quarantined until its root-mismatch failure is
   understood. It is a valid safety signal, not a deletion candidate.
