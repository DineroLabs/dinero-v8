...
| 6 | Reproducible builds | Reproducible mode now requires an explicit `SOURCE_DATE_EPOCH`, normalizes source/build paths and platform linker metadata, pins the Rust MSRV, compares independent artifacts, and runs two isolated builders in CI. |
