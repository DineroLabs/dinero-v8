...
| 6 | Reproducible builds | Reproducible mode now requires an explicit `SOURCE_DATE_EPOCH`, normalizes first-party and isolated vendored RocksDB source/build paths plus the compiled development schema fallback and Linux linker metadata, preserves the loader-required Mach-O UUID, pins the Rust MSRV, and compares two isolated Linux builders without depending on optional port-mapping downloads. |
