// ═══════════════════════════════════════════════════════════════════════════
// Chain Oracle Client Implementation (Phase 9.2: Production Oracles)
// ═══════════════════════════════════════════════════════════════════════════
...
ChainOracleClient::ChainOracleClient(const std::string& socket_path)
    : m_socket_path(socket_path)
    , m_socket_fd(-1)
{
}

ChainOracleClient::~ChainOracleClient() {
    disconnect();
}
...
bool ChainOracleClient::connect() {
...
void ChainOracleClient::disconnect() {
    if (m_socket_fd >= 0) {
        close(m_socket_fd);
        m_socket_fd = -1;
    }
...
bool ChainOracleClient::sendBlockConnected(uint64_t height, const std::string& block_hash) {
    // Serialize: BLOCK_CONNECTED height=<height> hash=<block_hash>
    std::ostringstream oss;
    oss << "BLOCK_CONNECTED height=" << height << " hash=" << block_hash;

    return sendMessage(oss.str());
}

bool ChainOracleClient::sendBlockDisconnected(uint64_t height, const std::string& block_hash) {
    // Serialize: BLOCK_DISCONNECTED height=<height> hash=<block_hash>
    std::ostringstream oss;
    oss << "BLOCK_DISCONNECTED height=" << height << " hash=" << block_hash;
...
bool ChainOracleClient::sendMessage(const std::string& message) {
...
bool ChainOracleClient::readResponse(std::string& response) {
    response.clear();
    ...
    while (true) {
        ...
        if (poll_result == 0) {
            continue;  // Timeout - keep waiting (up to 10 iterations = 1 second)
        }
        ...
        // Timeout after 10 iterations (1 second total)
...
namespace dinero {
namespace ipc {
ChainOracleClient::ChainOracleClient(const std::string&) : m_socket_fd(-1) {}
ChainOracleClient::~ChainOracleClient() {}
bool ChainOracleClient::connect() { return false; }
void ChainOracleClient::disconnect() {}
bool ChainOracleClient::sendBlockConnected(uint64_t, const std::string&) { return false; }
bool ChainOracleClient::sendBlockDisconnected(uint64_t, const std::string&) { return false; }
bool ChainOracleClient::sendMessage(const std::string&) { return false; }
bool ChainOracleClient::readResponse(std::string&) { return false; }
} // namespace ipc
} // namespace dinero
