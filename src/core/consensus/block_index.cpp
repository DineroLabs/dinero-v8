...
CBlockIndex* AddBlockIndex(const BlockHeader& header, uint32_t height) {
    ...
    if (!header.prev_block_hash.empty()) {
        raw_ptr->pprev = FindBlockIndex(header.prev_block_hash);
        if (raw_ptr->pprev) {
            raw_ptr->pprev->children.push_back(raw_ptr);
        }
    } else if (!header.prev_block_hash.empty()) {
        raw_ptr->pprev = FindBlockIndex(header.prev_block_hash);
        if (raw_ptr->pprev) {
            raw_ptr->pprev->children.push_back(raw_ptr);
        }
    ...
    dinero::g_logger.info("Added block: height=" + std::to_string(height) + 
                         " hash=" + hash.substr(0, 16) + "... work=..." + raw_ptr->chainwork.substr(48, 16));
...
void AddCandidate(CBlockIndex* block_index) {
    ...
    // Only consider blocks with BLOCK_VALID_CHAIN status as candidates
    if (!(block_index->status & BLOCK_VALID_CHAIN)) {
        return; // Will be added later when validation completes
    }
...
bool MaybeQueueOrphan(CBlockIndex* block_index) {
    ...
    if (!block_index->pprev) {
        std::string parent_hash = block_index->prev_hash;
        g_orphan_pool[parent_hash].push_back(block_index);
        
        dinero::g_logger.debug("Queued orphan: " + block_index->hash.substr(0, 16) + 
                              "... waiting for missing parent " + parent_hash.substr(0, 16) + "...");
        return true;
    }
    ...
    if (!(block_index->pprev->status & BLOCK_VALID_CHAIN)) {
        ...
        dinero::g_logger.info("Queued orphan: " + block_index->hash.substr(0, 16) + 
                             "... waiting for parent " + parent_hash.substr(0, 16) + "... to be validated");
        return true;
    }
...
void OnParentValidated(CBlockIndex* parent_index) {
    ...
    std::string parent_hash = parent_index->hash;
...
void ProcessOrphanQueue(const std::string& parent_hash) {
    ...
    if (it == g_orphan_pool.end()) {
        dinero::g_logger.debug("No orphans found for parent " + parent_hash.substr(0, 16) + "...");
        return;
    }
    ...
    dinero::g_logger.info("Processing " + std::to_string(orphans.size()) + 
                         " orphans for parent " + parent_hash.substr(0, 16) + "...");
    ...
    for (CBlockIndex* orphan : orphans) {
        if (CanConnect(orphan)) {
            ...
            dinero::g_logger.info("Connected orphan: " + orphan->hash.substr(0, 16) + 
                                 "... height=" + std::to_string(orphan->height));
            
            // Process any children of this orphan (cascade)
            dinero::g_logger.debug("Looking for children of " + orphan->hash.substr(0, 16) + "...");
...
void MarkBlockValid(CBlockIndex* block_index, uint32_t validation_flags) {
    ...
    dinero::g_logger.debug("Block " + block_index->hash.substr(0, 16) + 
