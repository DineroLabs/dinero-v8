...
CoinJoinAdapterJM::CoinJoinAdapterJM(std::string base_url) : base_url_(std::move(base_url)) {}

std::string CoinJoinAdapterJM::register_round(const CJJoinParams& p) {
    Json::Value req;
    ...
    if (!reader->parse(response.data(), response.data() + response.size(), &resp, &errors)) {
        throw std::runtime_error("JM bridge: bad register response: " + errors);
    }
    
    if (!resp.isMember("round_id")) {
        throw std::runtime_error("JM bridge: missing round_id in register response");
    }
    
    return resp["round_id"].asString();
}

void CoinJoinAdapterJM::submit_inputs(const std::string& rid,
...
CJStatusLite CoinJoinAdapterJM::status(const std::string& rid) {
    ...
    if (!reader->parse(response.data(), response.data() + response.size(), &resp, &errors)) {
        throw std::runtime_error("JM bridge: bad status response: " + errors);
    }
...
std::string CoinJoinAdapterJM::fetch_psbt(const std::string& rid) {
    ...
    if (!reader->parse(response.data(), response.data() + response.size(), &resp, &errors)) {
        throw std::runtime_error("JM bridge: bad psbt response: " + errors);
    }
    
    if (!resp.isMember("psbt")) {
        throw std::runtime_error("JM bridge: missing psbt in response");
    }
    
    return resp["psbt"].asString();
}

void CoinJoinAdapterJM::submit_signed(const std::string& rid, const std::string& psbt_b64) {
    Json::Value req;
    req["round_id"] = rid;
    req["psbt"] = psbt_b64;
...
void CoinJoinAdapterJM::cancel(const std::string& rid) {
    Json::Value req;
    req["round_id"] = rid;
...
std::string CoinJoinAdapterJM::http_post_json(const std::string& path, const std::string& body) {
    try {
        ...
        req.set(http::field::user_agent, "DineroCoinJoinJM/1.0");
        ...
        throw std::runtime_error("JM bridge HTTP POST failed: " + std::string(e.what()));
    }
}

std::string CoinJoinAdapterJM::http_get(const std::string& path) {
    try {
        boost::asio::io_context ioc;
        tcp::resolver resolver(ioc);
        tcp::socket socket(ioc);
        ...
        throw std::runtime_error("JM bridge HTTP GET failed: " + std::string(e.what()));
    }
}
