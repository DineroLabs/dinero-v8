    ...
    , currentTheme_("light")
{
    ...
    // Setup timers
    balanceTimer_->setInterval(30000); // 30 seconds
    syncTimer_->setInterval(60000); // 1 minute
    
    connect(balanceTimer_, &QTimer::timeout, this, &MobileWalletController::refreshBalance);
    connect(syncTimer_, &QTimer::timeout, this, &MobileWalletController::syncTransactions);
...
void MobileWalletController::syncTransactions() {
    ...
    recentTransactions_.append("tx_received_001");
    recentTransactions_.append("tx_sent_002");
    recentTransactions_.append("tx_received_003");
...
bool MobileWalletController::makeRpcCall(const QString& method, const QVariantMap& params) {
    QJsonObject request;
    request["jsonrpc"] = "2.0";
    ...
    QNetworkRequest req;
    req.setUrl(QUrl(rpcUrl_));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    
    QNetworkReply* reply = networkManager_->post(req, data);
    connect(reply, &QNetworkReply::finished, this, &MobileWalletController::onNetworkReply);
    
    return true;
}

void MobileWalletController::onNetworkReply() {
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        qWarning() << "RPC call failed:" << reply->errorString();
    }
    
    reply->deleteLater();
}

void MobileWalletController::parseRpcResponse(const QByteArray& response) {
    QJsonParseError error;
    QJsonDocument doc = QJsonDocument::fromJson(response, &error);
    
    if (error.error != QJsonParseError::NoError) {
        qWarning() << "Failed to parse RPC response:" << error.errorString();
        return;
    }
    ...
    if (result.contains("result")) {
        ...
        QJsonValue resultValue = result["result"];
        // Process result based on the method called
    } else if (result.contains("error")) {
        // Handle error response
        QJsonObject error = result["error"].toObject();
        qWarning() << "RPC error:" << error["message"].toString();
    }
}

void MobileWalletController::updateBalance() {
    // Mock implementation
    balance_ = 100.0;
    emit balanceChanged();
}

void MobileWalletController::updateTransactions() {
    // Mock implementation
    recentTransactions_.clear();
    recentTransactions_.append("tx_received_001");
    recentTransactions_.append("tx_sent_002");
...
QString MobileWalletController::generateQRCodeData(const QString& data) {
...
void MobileWalletController::onBalanceTimer() {
    refreshBalance();
}

void MobileWalletController::onSyncTimer() {
    syncTransactions();
}

void MobileWalletController::onBiometricAuthResult(bool success) {
    if (success) {
        walletLocked_ = false;
        emit walletUnlocked();
    } else {
