...
BlockAcceptResult BlockAcceptor::AcceptBlockFromRPC(const std::string& blockHex, const std::string& source) {
    try {
            ...
            dinero::uint256::FromHexUnsafe(block.prevBlockHash);
        {
            ...
            if (chainstate) {
                // ASSERT the lock; do not re-acquire it.
                //
                // AcceptBlockFromRPC takes activation_mutex_ at the top of the
                // function and holds it for the COMPLETE read/validate/store/
                // activate sequence, and AcceptBlockFromPeer routes through
                // that same function. So the classification below is not a
                // snapshot that can go stale: the tip cannot move between here
                // and the Utreexo root computation, which matters because that
                // computation reads the LIVE consensus_utxo_set_.
                //
                // This was NOT obvious -- it was mistaken for a TOCTOU twice,
                // and a whole reclassification mechanism was built for it
                // before a barrier test showed invalidateblock BLOCKING for the
                // entire park, i.e. the window does not exist. Asserting states
                // the invariant where a reader will look for it, and makes
                // removing the outer lock fail loudly rather than silently
                // reopening the window.
                chainstate->AssertActivationLockHeld("block classification");
