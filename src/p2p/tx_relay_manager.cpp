...
void TxRelayManager::OnPeerTx(const std::string& peer_id, const Transaction& tx) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // Early rejection of confidential transactions that would fail validation.
    // This saves CPU cycles and provides clear P2P-layer logging.
    // ═══════════════════════════════════════════════════════════════════════════
    if (tx.HasConfidentialOutputs()) {
        // Check 1: Kill-switch - reject if confidential txs are disabled
        if (dinero::Params().disable_confidential_transactions) {
            dinero::g_logger.warning("P2P: Rejecting confidential tx from " + peer_id +
                                    " (kill-switch engaged): " + txid);
            stats_.transactions_dropped++;
            return;
        }

        // Log confidential transaction relay
        dinero::g_logger.debug("P2P: Relaying confidential tx from " + peer_id + ": " + txid);
    }
