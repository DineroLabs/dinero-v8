...
#include "p2p_message.h"  // For p2p::init_p2p_network()
...
//
// The magic value comes from chainparams (single source of truth in
// src/consensus/chainparams_impl.cpp). We pick the right chain from
// the network name, drive SelectParams(), then read Params().magic
// back — exactly the same pattern as p2p::init_p2p_network(). The
// downstream ::p2p::init_p2p_network() call below picks the magic up
// the same way; the two stay aligned because both read from the same
// chainparams accessor.
